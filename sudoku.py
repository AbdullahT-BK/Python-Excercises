...
def find_next_empty(puzzle):
    # finds the nextrow, col on the puzzle that's not filled yet --- > rep with a -1
...
def is_valid(puzzle, guess, row, col):
    ...
    # and then the square
    # this is tricky, but we want to get where a 3x3 square starts
    ...
    for r in range(row_start, row_start +3):
        for c in range(col_start, col_start + 3):
            ...
            # if we get here, then these checks pass
            return True
...
def solve_sudoku(puzzle): # if no spaces in the puzzle are empty (-1)
